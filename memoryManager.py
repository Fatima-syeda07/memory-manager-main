...
class MemoryManager:
    def __init__(self, total_size=128, block_size=2):
        ...
        self.fifo_queue = []      # FIFO ke liye order track karega
        self.last_used = {}       # LRU ke liye timestamp
    ...
    def allocate(self, process, size, strategy="first"):
        ...
        if strategy == "first":
            ...
            return self._fifo_replacement(process, blocks_needed)
        elif strategy == "lru":
            return self._lru_replacement(process, blocks_needed)
        else:
            raise ValueError("Invalid Strategy! Use 'first', 'best', 'worst', 'fifo', 'lru'.")
    ...
    def _allocate_blocks(self, start, process, blocks_needed):
        ...
        for _ in range(blocks_needed):
            ...
            current = current.next
        self.last_used[process] = time.time()
        if process not in self.fifo_queue:
            self.fifo_queue.append(process)

    # FIFO REPLACEMENT
    def _fifo_replacement(self, process, blocks_needed):
        if not self.fifo_queue:
            return self._first_fit(process, blocks_needed)
        for _ in range(len(self.fifo_queue)):
            oldest = self.fifo_queue.pop(0)
            self.deallocate(oldest)
            if self._first_fit(process, blocks_needed):
                self.fifo_queue.append(process)
                return True
        return False

    # LRU REPLACEMENT
    def _lru_replacement(self, process, blocks_needed):
        if not self.last_used:
            return self._first_fit(process, blocks_needed)
        lru_process = min(self.last_used, key=self.last_used.get)
        self.deallocate(lru_process)
        del self.last_used[lru_process]
