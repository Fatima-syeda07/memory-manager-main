...
class MemoryGUI:
    def __init__(self, manager):
        ...
        self.root = tk.Tk()
        self.root.title("Memory Manager")  # English title (optional)
        ...
        # New page replacement buttons
        tk.Button(frame, text="FIFO", command=lambda: self.allocate("fifo")).grid(row=0, column=8)
        tk.Button(frame, text="LRU", command=lambda: self.allocate("lru")).grid(row=0, column=9)

        tk.Button(frame, text="Deallocate", command=self.deallocate).grid(row=0, column=7)
